...
export var SampleApp = (function () {
  ...
  let latestProcessor: LivenessCheckProcessor | null = null;
  ...
  const loadAssets = () => {
    ...
    if (!resultProductKey) {
      SampleAppUtilities.displayStatus(
        'Não foi possível obter a chave de produção. Retorne à home e gere uma nova appkey.'
      );
    ...
    // Initialize FaceTec Browser SDK exactly once (via the autogenerated config,
    // which internally calls FaceTecSDK.initializeInProductionMode). Do NOT also
    // call initializeInProductionMode separately: two overlapping inits race and
    // leave the SDK in InitializationNotCompleted when a session starts.
  ...
  const getSessionToken = async (facecaptchaService: any, appkey: any) => {
    ...
    await facecaptchaService.getSessionToken(appkey, getUserAgent()).subscribe(
      (res: any) => {
